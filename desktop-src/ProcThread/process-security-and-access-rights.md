...
| **PROCESS\_DUP\_HANDLE** (0x0040)                 | Required to duplicate a handle using [**DuplicateHandle**](https://msdn.microsoft.com/library/ms724251(v=VS.85).aspx). **Warning**: **PROCESS\_DUP\_HANDLE** can be used to gain complete control over another process. See the **Remarks** section for more information. |
...
To open a handle to a process that you would not otherwise have access to (for example, a process running under a different account), you must enable the **SeDebugPrivilege** privilege. For more information, see [Changing Privileges in a Token](https://msdn.microsoft.com/library/ms717797(v=VS.85).aspx).
...
A process with **PROCESS\_DUP\_HANDLE** access to another process can cause security or reliability issues by closing existing handles and attempting to re-use the same handle value for a different purpose. You should never grant **PROCESS\_DUP\_HANDLE** to a lower-privileged process except in well-understood cases. For example, a regular Medium-IL process could grant **PROCESS\_DUP\_HANDLE** to a trusted child process running in an AppContainer as part of a run-with-least-privilege mitigation. Although the child process *could* attack the main process by abusing **PROCESS\_DUP\_HANDLE**, this is no worse than if the child process had been created at Medium-IL in the first place. For more information on integrity levels, see **[Mandatory Integrity Control](/windows/win32/secauthz/mandatory-integrity-control)**.
